...
- **[curriculum-extensibility.md](./curriculum-extensibility.md)** — the curriculum data-model design (decided, implemented) for katakana, sokuon, chōon, yōon, and 特殊音, plus a "Progress" log of how each category actually landed. Predates Special Katakana's reintroduction; see the note at the top of that doc and `CLAUDE.md` for current status.
- **[definition-of-done.md](./definition-of-done.md)** — what "Done" means for a non-trivial task.
- **[ai-development-loop.md](./ai-development-loop.md)** — the Explore → Plan → Implement → Verify → PR loop and the `/kana-task` Skill it backs.
