...
**Status: design decided in conversation (2026-08-14/15). Katakana and sokuon implemented — see "Progress" below. Chōon/yōon/特殊音 not yet started.** The open questions from the first draft of this doc have been resolved by the user directly; this version reflects those decisions.
...
- **長音/拗音/特殊音 — not started.** 長音 (chōon) reuses the exact same `learnStyle: 'contrast-pairs'` infrastructure sokuon just built — see "Next step" below. 拗音/特殊音 are `'character-set'` but still need the "one kana glyph = one mora" fix noted below before either can ship real content.
...
促音 introduces one real new glyph (っ) — it needed a `CHARACTERS` entry, stroke data, `CONFUSABLE_PAIRS` (vs つ/ツ), like any other character, just taught via word contrast rather than solo flashcard (done in `feature/sokuon`). 長音, though, might introduce **none**: ー is already taught fresh under カタカナ単音 (it's part of learning katakana itself, not its own lesson), and hiragana's long vowels reuse existing vowel characters — 長音's own lesson only ever *reviews* ー, never introduces it. If that's right, 長音 is a row with `characterIds: []` (or no new-character step at all) — `TracingPage.tsx`'s early-return now checks `words.length` instead of `charPool.length` for `'contrast-pairs'` rows specifically (see above), so this case is already handled; `LearnPage.tsx`'s skip-to-step-B behavior doesn't depend on `characterIds` being non-empty either. Still worth double-checking directly against real 長音 content when that work starts, rather than assuming.
...
The design is fully decided; katakana (`'character-set'`) and sokuon (the first `'contrast-pairs'` category) are both implemented — see "Progress" above. What's left:

- **長音 (chōon) — next up.** Reuses `learnStyle: 'contrast-pairs'` exactly as-is; this should be pure content work (characters — possibly none, see "Remaining structural note" above — a row, vocabulary spanning both scripts per the design) with zero further Learn/Practice/Tracing code, same way sokuon's own content work was, once its own flow generalization commit landed. `sokuon-row` in `words.ts` and `feature/sokuon`'s commits are the worked example to follow.
- **拗音 (yōon) and 特殊音 — after that.** Both `'character-set'`, same shape as katakana, but blocked on the one-kana-glyph-= one-mora accent fix noted above before real content can ship with correct pitch-accent lines.
