...
async function main() {
  ...
  const variants: [string, string][] = [
    ['kanji-period', '<phoneme alphabet="sapi" ph="イエ">家。</phoneme>'],
    ['kanji-noperiod', '<phoneme alphabet="sapi" ph="イエ">家</phoneme>'],
    ['kana-period', '<phoneme alphabet="sapi" ph="イエ">いえ。</phoneme>'],
    ['kana-noperiod', '<phoneme alphabet="sapi" ph="イエ">いえ</phoneme>'],
    ['plain-kanji-period', '家。'],
    ['plain-kana-period', 'いえ。'],
  ]

  for (const [label, inner] of variants) {
    const buf = await synthesizeSSML(wrapSSML(inner), key, region)
    const outPath = `${OUT_DIR}/a-ie-${label}.wav`
