// ja-JP-Nanami:DragonHDLatestNeural — Azure's newer HD voice, which turned
// out to nail intonation on a few words the regular NanamiNeural struggled
// with (有名, ピュア) where SAPI phoneme notation alone couldn't force
// correct prosody. Used surgically for just those words, not as the
// project-wide default voice (untested at scale, unclear commercial-tier
// interaction with HD voices specifically — see high-definition-voices.md).
...
async function main() {
  ...
  for (const id of ids) {
    ...
    console.log(`  words/${w.id}.wav  ("${text}") [DragonHD]`)
  }
}
