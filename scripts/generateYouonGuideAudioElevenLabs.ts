// Generates the six Yōon Guide narration clips (Issue #50). The locale
...
// audioKey, one clip per step.
//
//   ELEVENLABS_API_KEY=sk_... npx tsx scripts/generateYouonGuideAudioElevenLabs.ts
//   ELEVENLABS_API_KEY=sk_... npx tsx scripts/generateYouonGuideAudioElevenLabs.ts --regenerate
import { copyFile, mkdir, writeFile } from 'node:fs/promises'
import path from 'node:path'
import { DEFAULT_YOUON_GUIDE_LOCALE, YOUON_GUIDE_CONTENT } from '../src/data/youonGuideContent'
import { fileExists, requireApiKey, synthesize } from './elevenLabsClient'

const TAMAMIZU_VOICE_ID = 'L7lwGdJEpZzBrRbKVqzV'
const content = YOUON_GUIDE_CONTENT[DEFAULT_YOUON_GUIDE_LOCALE]
...
async function main() {
  ...
  for (const step of Object.values(content.steps)) {
    // audioKey is e.g. 'guide/youon-one' -> file basename 'youon-one.wav'.
