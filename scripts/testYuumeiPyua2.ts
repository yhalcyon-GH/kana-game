...
async function main() {
  ...
  for (let i = 0; i < 4; i++) {
    const buf = await synthesizeSSML(wrapSSML('<phoneme alphabet="sapi" ph="ユウメー">有名</phoneme>。'), key, region)
    await writeFile(`${OUT_DIR}/yuumei-take${i}.wav`, buf)
    console.log(`  yuumei-take${i}`)
  }

  const pyuaVariants: [string, string][] = [
    ['pyua-phua', 'Phua。'],
    ['pyua-hiragana', 'ぴゅあ。'],
  ]
  for (const [label, inner] of pyuaVariants) {
