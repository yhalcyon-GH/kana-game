 ...
 *
 * POST /api/dev-only/last-login-code.php {"email": "<normalized-email>"}
 ...
 * from READING the response, but a cross-site GET is a browser "simple
 * request" -- it still reaches the server and still lets
 * DevHarnessLoginCodeStore::consume() delete the pending row as a side
 * effect, even though the attacker can never see the result. POST + a
 * required `application/json` body closes this: a hostile cross-site
 * <form> cannot set an `application/json` Content-Type (form encodings
 * are limited to the three CORS-simple MIME types), and a hostile
 * cross-site `fetch`/XHR that DOES set one becomes CORS-preflighted, so
 * the browser only sends the real request once
 * Cors::applyPreflightHeaders() has already allowlisted the Origin. The
 * explicit Origin re-check below is defense-in-depth for any direct
 * (non-browser) caller that supplies a spoofed Origin header without
 * going through preflight at all.
 *
 ...
 * last-magic-link.php) -- this is now a non-"simple" request
 * (application/json body), so a browser sends a real OPTIONS preflight
 * first; applyPreflightHeaders() answers it with the allowed
 * methods/headers for an allowlisted Origin and nothing (no CORS
 * headers) otherwise, which is what makes the browser refuse to send
 * the follow-up POST from a disallowed Origin at all.
 */
