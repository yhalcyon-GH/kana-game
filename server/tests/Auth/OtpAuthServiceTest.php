...
function otpAuthServiceTests(): array
{
    return [
        ...
        'verifyCode() at the cap evicts via the atomic evictLruForUserIfAtCap() and the 4th login succeeds with the cap held' => function () {
            // verifyCode() now delegates cap enforcement entirely to
            // PersistentSessionRepository::evictLruForUserIfAtCap(),
            // which uses a single SELECT ... FOR UPDATE locking read
            // (see that method's own doc comment for why this, not a
            // count-then-evict-then-retry, is what actually closes the
            // race under MariaDB's default REPEATABLE READ isolation).
            // FOR UPDATE has no meaningful effect on the SQLite
            // connection this unit test runs against (single-writer,
            // already serialized), so this test proves the CALLING
            // CONTRACT is correct -- verifyCode() calls the atomic
            // method once, uses its result, and the cap holds for a
            // single sequential run. The DEFINITIVE proof that the cap
            // holds under real concurrent transactions is the MariaDB
            // scenario E harness in server/tests/mariadb-concurrency/
            // (not runnable here).
            $h = makeOtpAuthServiceHarness(null, 10, 10, 5, 3);
            ...
            assertTrue($fourthResult->success, 'the 4th login must still succeed');
            assertSame(3, $h['persistentSessions']->countActiveForUser($userId), 'active persistent sessions must still be exactly 3 after the 4th login evicts the oldest');
        },
