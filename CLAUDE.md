...
All six planned script categories exist now: hiragana, katakana, sokuon (促音, the small-tsu gemination mark), chōon (長音, long vowels), yōon (拗音, contracted sounds like きゃ/kya), and 特殊音 (tokushuon, extended katakana combinations for loanword sounds like ファ/ティ/ヴ — see `docs/curriculum-extensibility.md` for the full `ScriptCategory` design). **This was the last of the five originally-planned new categories (on top of hiragana) — each still lives on its own unmerged branch (`feature/katakana` → `feature/sokuon` → `feature/chouon` → `feature/youon` → `feature/tokushuon`), awaiting review; any further category work beyond these six is a new decision, not something already scoped.** Hiragana, katakana, yōon, and 特殊音 use `learnStyle: 'character-set'` (flashcard → recap → words, all four mini-games). Sokuon and chōon are both `learnStyle: 'contrast-pairs'`: Learn listens through minimal-pair words (おと vs おっと; おばさん vs おばあさん) instead of flashcarding a character, Tracing is word-level only, and Practice drops Kana Quiz. `LearnPage.tsx`/`PracticeHubPage.tsx`/`TracingPage.tsx` all branch on `learnStyle` (not the category id). Chōon is also the first row with `characterIds: []` — it introduces no new characters of its own (hiragana spells long vowels by repeating/extending existing vowel kana; katakana's ー was already taught under カタカナ単音) — every place that reads a row's characters already branches on `learnStyle` first, so this needed no code changes, only content (see `curriculum.test.ts` and `App.test.tsx`'s zero-new-character coverage). Yōon and 特殊音 are both `character-set` like katakana (pure content, no flow changes needed) but break the "one kana glyph = one mora" assumption baked into pitch-accent rendering (きゃ is 2 glyphs, 1 mora; ファ likewise) — see "Known gaps" below. 特殊音 is also the one category that's genuinely katakana-only (no hiragana equivalent exists for this mechanism) and depends on `sokuon` (not just `katakana`) since real vocabulary like ジェットコースター needs っ/ッ.
...
**One kana glyph = one mora, EXCEPT yōon (拗音) and 特殊音 (tokushuon).** Pitch-accent rendering (`WordCard.tsx`'s `AccentedKana`, built from `accents.ts`) assumes one glyph = one mora = one accent position — true for hiragana/katakana/sokuon/chōon content, but false for yōon (きゃ is 2 glyphs, 1 mora) and 特殊音 (ファ likewise; ヴ alone is the one 特殊音 exception that IS 1 glyph = 1 mora). This was deliberately **not** fixed at the rendering-model level (a real fix — accent keyed by mora, not glyph — is real, non-trivial, out-of-scope work) — instead, `AccentedKana` already had a length-mismatch guard (`accent.length !== n` → render plain kana, no accent line) that turned out to be sufficient: it just silently omits the accent line for yōon/特殊音 words rather than crashing or misrendering one. `scripts/buildAccentData.mjs` independently guards the same mismatch on the generation side (drops any resolved entry whose accent length disagrees with its kana's glyph length), so no yōon or 特殊音 word ever gets a written `ACCENT_PATTERNS` entry in the first place — see `WordCard.test.tsx` for tests proving this end-to-end (confirmed empirically for 特殊音 too: re-running `scripts/buildAccentData.mjs` after adding its content produced a byte-identical `accents.ts`, zero new entries). The stroke-order side has the same shape: `StrokeOrderAnimation.tsx`'s `STROKE_PATHS[characterId] ?? []` already defaults to an empty guide for a character with no data, and yōon/特殊音 characters were deliberately **never run through** `scripts/fetchStrokeData.ts` — that script keys a fetch off `kana.codePointAt(0)` (the first glyph only), so running it for a 2-glyph id like `kya` or `katakana-fa` would silently write き's or フ's stroke path mislabeled as きゃ's or ファ's, which is worse than no data at all — see `StrokeOrderAnimation.test.tsx`.
...
- **Home** (`HomePage`, route `/`) is a top-level chooser with three cards — ひらがな (`/hiragana`), カタカナ (`/katakana`), そのほか (`/other`) — not a page of rows itself. This replaced an earlier single flat page that stacked every category's rows together; it was split once enough categories existed that one page got unwieldy.
- **Script pages** (`CategoryRowsPage`, shared by all three routes above) show every row as a card (locked/new/taught/mastered), always navigable — rows are never access-gated, only progress-badged. `/hiragana` and `/katakana` each pass a single categoryId; `/other` bundles every category that isn't hiragana/katakana (`App.tsx`'s `OTHER_CATEGORY_IDS`, computed from `CATEGORIES` so a newly-added category just appears there with no route change) — currently 促音/長音/拗音/特殊音.
...
- **New content within 特殊音 (tokushuon)**: same shape again — `'character-set'`, pure content, same do-NOT-run-`fetchStrokeData.ts` rule as yōon. All six planned categories now exist; adding a 24th+ combination here (or a genuinely new 7th category) is new scope, not something this doc anticipates — see `docs/curriculum-extensibility.md`'s "Progress" section for exactly which combinations were included/excluded and why.
- **Stroke order data**: `src/data/strokes.ts` is generated by `scripts/fetchStrokeData.ts` from KanjiVG (CC BY-SA 3.0) — don't hand-edit, re-run the script. Object keys are `JSON.stringify`-escaped (needed once ids stopped being bare identifiers, e.g. `katakana-a`) — don't revert that to save characters. **Exception: never run it for a yōon/特殊音 (or future multi-glyph) character** — see above.
...
- **`ScriptCategory`/`categoryId` is the explicit scope tag now** (hiragana, katakana, sokuon, chōon, yōon, and tokushuon all exist — see "Data model" above, and see `docs/curriculum-extensibility.md` for how each landed). `learnStyle` drives the Learn/Practice/Tracing shape generically (`'character-set'` vs `'contrast-pairs'`) — don't special-case a category id directly in route/flow code; branch on `learnStyle` instead. This is exactly why 特殊音 needed zero Learn/Practice/Tracing code changes, same as yōon before it.
