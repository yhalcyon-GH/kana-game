...
function build(stage: RestaurantStageId, rows: (string | number | string[])[][]) { return rows.map((r) => dish(stage, r[0] as string, r[1] as string, r[2] as string, r[3] as number, r[4] as string[])) }
export const RESTAURANT_DISHES = [...build('hiragana', h), ...build('katakana', k), ...build('other', o), ...build('special-katakana', s)]
